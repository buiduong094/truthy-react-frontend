...
import {
  ...
  QUERY_TEMPLATE,
  SUBMIT_FORM,
} from 'containers/LabelCustom/constants';
...
export function* handleSubmitForm() {
  ...
  const requestUrl = `/label-customs${formMethod === PUT ? `/${id}` : ''}`;
...
export function* handleDeleteItemById(data) {
  ...
  const requestUrl = `/label-customs/${data.id}`;
...
export function* handleQueryTemplate() {
  ...
  const requestUrl = `/label-customs?${queryString}`;
...
export function* handleGetTemplateById() {
  yield put(asyncStartAction());
  const id = yield select(makeUpdateIdSelector());
  const requestUrl = `/label-customs/${id}`;
  const payload = ApiEndpoint.makeApiPayload(requestUrl, GET);
  try {
    const response = yield call(request, payload);
